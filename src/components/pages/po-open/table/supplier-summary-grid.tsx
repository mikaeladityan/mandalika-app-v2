...
import { useOpenPoSummary } from "@/app/(application)/po/open/server/use.po-open";
import { QueryOpenPoDTO } from "@/app/(application)/po/open/server/po-open.schema";
import { Card, CardContent } from "@/components/ui/card";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { Button } from "@/components/ui/button";
import { formatNumber } from "@/lib/utils";
import { Store, Download, Package, ChevronRight, Calculator, FileText, Hash } from "lucide-react";
import dayjs from "dayjs";
import { OpenPoService } from "@/app/(application)/po/open/server/po-open.service";

interface SupplierSummaryGridProps {
    queryParams: QueryOpenPoDTO;
}

export function SupplierSummaryGrid({ queryParams }: SupplierSummaryGridProps) {
    const { summary } = useOpenPoSummary(queryParams);

    const exportSingleSupplier = async (supplier: any) => {
        try {
            const data = await OpenPoService.export({
                ...queryParams,
            ...
            link.href = url;
            link.setAttribute("download", `PO_Open_${supplier.supplier_name}_${dayjs().format("YYYY-MM-DD")}.xlsx`);
        ...
        return (
            <div className="flex flex-col items-center justify-center py-20 bg-slate-50/50 rounded-[3rem] border-2 border-dashed border-slate-100 gap-4">
                ...
                <p className="text-sm font-bold text-slate-400 font-mono tracking-tight text-center">
                    Belum ada data PO Open untuk periode ini.
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="grid grid-cols-1 2xl:grid-cols-2 gap-8">
            {summary.data.map((supplier: any) => (
                <div key={supplier.supplier_id} className="group relative flex flex-col bg-white border border-slate-100 rounded-[2.5rem] shadow-sm hover:shadow-2xl hover:shadow-blue-50 hover:border-blue-100 transition-all duration-500 overflow-hidden">
                    {/* Card Header */}
                    <div className="p-8 pb-4 flex items-start justify-between">
                        <div className="flex items-center gap-5">
                            <div className="p-4 bg-blue-600 text-white rounded-[1.5rem] shadow-lg shadow-blue-100 group-hover:scale-110 transition-transform duration-500">
                    ...
                    <div className="mx-8 mb-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-between text-[10px] font-black text-slate-500 uppercase tracking-widest">
                        <div className="flex items-center gap-2">
                            ...
                            Status: <span className="text-blue-600">Active PO</span>
                        </div>
                        <div className="flex items-center gap-2">
                        ...
                        <Table>
                            <TableHeader className="bg-slate-50/50">
                                <TableRow className="hover:bg-transparent border-none">
                                    ...
                                    <TableHead className="text-[10px] font-black uppercase text-slate-400 tracking-wider h-10 text-center">Qty Open</TableHead>
                                    ...
                                    <TableRow key={idx} className="border-slate-50/50 hover:bg-slate-50/30 transition-colors group/row">
                                        <TableCell className="py-4 px-8">
                                            <div className="flex flex-col">
                                                <span className="text-[13px] font-bold text-slate-700 group-hover/row:text-blue-600 transition-colors">
                                                    {item.material_name}
                                                </span>
                    ...
                    <div className="p-8 pt-6 border-t border-slate-50 flex items-center justify-between bg-slate-50/20">
                        <Button 
                            variant="ghost" 
                            className="rounded-xl font-bold text-slate-400 hover:text-blue-600 transition-colors text-xs"
                        >
                            View Details
                        </Button>
                        <Button
                            onClick={() => exportSingleSupplier(supplier)}
                            className="h-12 rounded-[1.25rem] bg-white border border-blue-200 text-blue-600 hover:bg-blue-600 hover:text-white transition-all duration-300 font-black text-xs px-6 shadow-sm flex items-center gap-2 group/btn"
                        >
