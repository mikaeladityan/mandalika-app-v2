...
import { ArrowDown, ArrowUp, ChevronRight, Flag, Minus } from "lucide-react";
...
type SalesRow = Omit<ResponseSalesDTO, "quantity"> & {
    quantity: Array<{ year: number; month: number; quantity: number; trend: string }>;
};
...
export const SalesColumns = ({
    ...
    onSort,
    periods,
}: Props): ColumnDef<SalesRow>[] => {
    const now = new Date();
    const currentMonth = now.getMonth() + 1;
    const currentYear = now.getFullYear();
    const monthColumns: ColumnDef<SalesRow>[] = periods.map((p) => ({
        ...
        cell: ({ row }) => {
            ...
            if (!found?.quantity) {
                return <div className="text-xs text-muted-foreground text-start">–</div>;
            }
            ...
            return (
                <Link
                    ...
                    className="
                    cursor-button group w-full rounded-md pe-2 py-1.5 flex items-center justify-start gap-1 hover:bg-muted/60 transition-colors"
                >
                    <span className="text-sm font-semibold leading-none">
                        {Math.round(Number(found.quantity))}
                    </span>
    ...
    return [
        {
            ...
            cell: ({ row }) => (
                <div className="overflow-hidden">
                    ...
                    <p className="font-bold text-xs 2xl:text-base truncate leading-tight">
                        {row.original.product.name}
                    </p>
                    <div className="space-x-1">
                        <span className="text-[10px] 2xl:text-xs bg-secondary px-1 rounded uppercase">
