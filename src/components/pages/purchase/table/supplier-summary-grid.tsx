...
export function SupplierSummaryGrid({ queryParams }: SupplierSummaryGridProps) {
    ...
    const exportSingleSupplier = (supplier: PurchaseSummaryResponse) => {
        const exportData = supplier.items.map((item, idx) => ({
            No: idx + 1,
        ...
        exportData.push({
            ...
            UOM: "",
            Price: null,
            Subtotal: supplier.total_amount,
            Status: "",
        } as any);
        ...
        XLSX.writeFile(wb, `Order_${supplier.supplier_name}_${dayjs().format("YYYY-MM-DD")}.xlsx`);
    };
    ...
    return (
        <div className="grid grid-cols-1 2xl:grid-cols-2 gap-8">
            {summary.data.map((supplier) => (
                <div key={supplier.supplier_id} className="group relative flex flex-col bg-white border border-slate-100 rounded-[2.5rem] shadow-sm hover:shadow-2xl hover:shadow-indigo-50 hover:border-indigo-100 transition-all duration-500 overflow-hidden">
                    {/* Card Header */}
                    <div className="p-8 pb-4 flex items-start justify-between">
                        <div className="flex items-center gap-5">
                            <div className="p-4 bg-indigo-600 text-white rounded-[1.5rem] shadow-lg shadow-indigo-100 group-hover:scale-110 transition-transform duration-500">
                    ...
                    <div className="mx-8 mb-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-between text-[10px] font-black text-slate-500 uppercase tracking-widest">
                        <div className="flex items-center gap-2">
                            ...
                            Status: <span className="text-indigo-600">Active Procurement</span>
                        </div>
                        <div className="flex items-center gap-2">
                                    ...
                                    <TableRow key={idx} className="border-slate-50/50 hover:bg-slate-50/30 transition-colors group/row">
                                        <TableCell className="py-4 px-8">
                                            <div className="flex flex-col">
                                                <span className="text-[13px] font-bold text-slate-700 group-hover/row:text-indigo-600 transition-colors">
                                                    {item.material_name}
                                                </span>
                    ...
                    <div className="p-8 pt-6 border-t border-slate-50 flex items-center justify-between bg-slate-50/20">
                        <Button 
                            variant="ghost" 
                            className="rounded-xl font-bold text-slate-400 hover:text-indigo-600 transition-colors text-xs"
                        >
                            View All Items
                        </Button>
                        <Button
                            onClick={() => exportSingleSupplier(supplier)}
                            className="h-12 rounded-[1.25rem] bg-white border border-indigo-200 text-indigo-600 hover:bg-indigo-600 hover:text-white transition-all duration-300 font-black text-xs px-6 shadow-sm flex items-center gap-2 group/btn"
                        >
