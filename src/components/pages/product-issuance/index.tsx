...
import { useIssuanceQuery, useIssuanceTableState } from "@/app/(application)/product-issuance/server/use.issuance";
...
import { IssuanceColumns } from "./table/column";
...
export function Issuance() {
    const table = useIssuanceTableState();
    const [columnVisibility, setColumnVisibility] = useLocalStorage<VisibilityState>(
        "issuance-main-table-visibility",
        {},
    );
    ...
    const {
        ...
        isFetching,
        isRefetching,
    } = useIssuanceQuery(table.queryParams);
    ...
    const periods = useMemo(() => {
        if (!issuanceList || issuanceList.length === 0) return [];
        return issuanceList[0].quantity.map((q) => ({
            year: q.year,
            month: q.month,
        }));
    }, [issuanceList]);
        ...
        <section className="flex flex-col gap-4">
            <Card className="border-none shadow-sm rounded-xl overflow-hidden bg-white">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-xl font-bold text-foreground tracking-tight">
                                    Manajemen Pengeluaran {typeLabel !== "Semua Channel" ? `— ${typeLabel}` : ""}
                                </h1>
                                <p className="text-xs text-muted-foreground font-medium">
                                    Monitoring tren pengeluaran {typeLabel.toLowerCase()} produk secara aktual.
                                </p>
                            </div>
                        </div>
                        <div className="flex flex-wrap items-center gap-2">
                            <Link href={`/product-issuance/create${table.type ? `?type=${table.type}` : ""}`}>
                                <Button
                                    variant="default"
                            ...
                            <Link href={`/product-issuance/import${table.type ? `?type=${table.type}` : ""}`}>
                                <Button
                                    variant="outline"
                                    className="h-9 px-4 font-semibold"
                                >
                ...
                <CardContent className="relative">
                    ...
                    {isTableLoading && !issuanceList?.length ? (
                        <TableSkeleton />
                    ) : (
                        <DataTable
                            tableId="issuance-main-table"
                            columns={columns}
                            data={issuanceList || []}
