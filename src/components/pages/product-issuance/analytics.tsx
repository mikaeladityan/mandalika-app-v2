...
export function IssuanceAnalytics() {
    const table = useIssuanceTableState();
    const { issuances, isLoading, isFetching } = useIssuanceQuery(table.queryParams);
    ...
    const chartData = useMemo(() => {
        if (!issuances?.issuances || issuances.issuances.length === 0) return [];
        ...
        const periods = issuances.issuances[0].quantity.map((q: any) => ({
            month: q.month,
            year: q.year,
        }));
    ...
    const summaries = useMemo(() => {
        ...
        return issuances.issuances.map((s: any, idx: number) => {
                                    ...
                                    >
                                        <div className="truncate flex items-center gap-1.5 flex-wrap">
                                            {(() => {
                                                ...
                                                const sItem = issuances?.issuances?.find(
                ...
                <div className="grid gap-4 md:grid-cols-3">
                    ...
                    <Card
                        className={cn(
                            ...
                            summaries.length > 1 ? "bg-indigo-50/30" : "bg-background",
                        )}
                    >
                        <CardHeader className="pb-2">
