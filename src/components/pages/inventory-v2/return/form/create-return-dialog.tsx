...
import { CreateReturnBody } from "./create-return-body";
...
export function CreateReturnDialog({ open, onOpenChange, isMobile }: CreateReturnDialogProps) {
    if (isMobile) {
        return (
            <Dialog open={open} onOpenChange={onOpenChange}>
                <DialogContent className="w-screen h-dvh max-w-none m-0 p-4 pt-10 rounded-none overflow-y-auto bg-slate-50 border-none flex flex-col items-center">
                    <DialogHeader className="sr-only">
                        <DialogTitle>Buat Pengembalian Barang (Retur)</DialogTitle>
                    </DialogHeader>
