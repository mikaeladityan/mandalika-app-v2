...
} from "./server/outlet-inventory.schema";
import { OutletInventoryService } from "./server/outlet-inventory.service";
...
export function useActionOutletInventory(outletId: number) {
    ...
    >({
        ...
        mutationFn: ({ productId, body }) =>
            OutletInventoryService.setMinStock(outletId, productId, body),
        onSuccess: () => {
            setNotif({
                title: "Stok Minimum",
