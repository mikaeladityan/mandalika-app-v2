...
export class RawMatService {
    ...
    static async bulkStatus(ids: number[], status: "ACTIVE" | "DELETE") {
        try {
            await setupCSRFToken();
            const { data } = await api.put<ApiSuccessResponse<void>>(`${API}/bulk-status`, {
                ids,
                status,
            });
            return data.data;
    ...
    static async export(params: QueryRawMaterialDTO) {
        try {
            await setupCSRFToken();
            const { data } = await api.get<Blob>(`${API}/export`, {
                params,
                responseType: "blob",
            });
            return data;
